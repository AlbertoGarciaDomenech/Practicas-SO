...
static int my_unlink(const char *path){
	fprintf(stderr, "--->>> my_unlink: path %s",path+1);
	//funciones copiadas de mknod

	//we have to get the i-node we want to remove
	int iNodeIndex, inode;

	if(iNodeIndex = (findFileByName(&myFileSystem,(char *)path+1)) == -1)
		return -ENOENT;	//check if it exists
	inode = myFileSystem.directory.files[iNodeIndex].nodeIdx;


	//change directory
	myFileSystem.directory.files[iNodeIndex].freeFile = true;
    myFileSystem.directory.numFiles--;
    //change inode
  	if(resizeNode(inode,0) < 0)
  		return -EIO;
    myFileSystem.nodes[inode]->fileSize = 0;
    myFileSystem.nodes[inode]->numBlocks = 0;
    myFileSystem.nodes[inode]->freeNode = true;

    myFileSystem.numFreeNodes++;
    myFileSystem.numFreeNodes = myQuota(&myFileSystem);
    //update filesystem
    updateBitmap(&myFileSystem);
    updateDirectory(&myFileSystem);
    updateNode(&myFileSystem, iNodeIndex, myFileSystem.nodes[iNodeIndex]);
    updateSuperBlock(&myFileSystem);
